...
export interface EmbedResult {
  ...
  dryRun: boolean;
  /** Fork: number of page-level embedding failures encountered during the run. */
  failed_pages: number;
  /** Fork: sampled page-level embedding error messages for diagnostics surfaces. */
  errors: string[];
  /**
...
async function embedAll(
  ...
  async function embedOnePage(page: typeof pages[number]) {
    ...
    try {
      ...
      result.failed_pages++;
      const msg = `Error embedding ${page.slug}: ${e instanceof Error ? e.message : e}`;
      if (result.errors.length < 10) result.errors.push(msg);
      serr(`\n  ${msg}`);
    }
