...
export function runGbrainSubprocess(cmd: string, opts?: { timeoutMs?: number }): string {
  // Render/source-checkout runs have no global `gbrain` binary. Rewrite the
  // leading `gbrain` token to the resolved CLI entrypoint (compiled binary,
  // `bun run src/cli.ts`, or plain `gbrain` when globally linked) so backfill
  // phases work wherever apply-migrations itself was launched from.
  const resolved = cmd.replace(/^gbrain(?=\s|$)/, migrationCliCommand());
  try {
    const out = execSync(resolved, {
